...
export default function CallsPage() {
  ...
  return (
    <div className="flex h-full w-full bg-[var(--bg-main)] text-[var(--text-primary)] select-none overflow-hidden transition-colors">
      {/* Calls Sidebar / List Pane */}
      <div className="w-full md:w-80 lg:w-[360px] flex flex-col border-r border-[var(--border-subtle)] h-full bg-[var(--bg-sidebar)] transition-colors">
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-4 pb-3">
          <h1 className="text-[22px] font-bold text-[var(--text-primary)] tracking-tight">Calls</h1>
        ...
        <div className="px-3 pt-2">
          <button
            ...
            className="w-full flex items-center gap-3.5 px-3 py-2.5 rounded-[12px] hover:bg-[var(--bg-hover)] text-left transition-colors group"
          >
            <div className="w-10 h-10 rounded-full bg-[var(--bg-pill)] flex items-center justify-center text-[var(--text-primary)] shrink-0 group-hover:bg-[var(--bg-active)] transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
            ...
            <div className="flex flex-col">
              <span className="text-[15px] font-medium text-[var(--text-primary)]">Create a Call Link</span>
            </div>
          </button>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          <h2 className="text-[17px] font-bold text-[var(--text-primary)] mb-1">No calls</h2>
          <p className="text-[13px] text-[var(--text-secondary)]">Recent calls will appear here.</p>
        </div>
      </div>

      {/* Right Main Placeholder Pane */}
      <div className="hidden md:flex flex-1 flex-col items-center justify-center bg-[var(--bg-main)] p-8 text-center transition-colors">
        <div className="w-16 h-16 rounded-full bg-[var(--bg-card)] border border-[var(--border-subtle)] flex items-center justify-center text-[var(--text-secondary)] mb-3 shadow-sm">
        ...
        <h3 className="text-[16px] font-semibold text-[var(--text-primary)] mb-1">Make a Call</h3>
      ...
      {callLinkModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-[16px] p-6 max-w-md w-full shadow-2xl animate-in fade-in zoom-in-95 duration-150 text-[var(--text-primary)]">
            <h3 className="text-lg font-bold text-[var(--text-primary)] mb-2">Create a Call Link</h3>
            <p className="text-xs text-[var(--text-secondary)] mb-4">
              Anyone with this link can join your call on Signal with end-to-end encryption.
            </p>
            <div className="flex items-center gap-2 bg-[var(--bg-pill)] border border-[var(--border-subtle)] rounded-xl p-2.5 mb-5">
              <span className="text-xs text-[var(--text-primary)] font-mono truncate flex-1">{mockLink}</span>
              <button
